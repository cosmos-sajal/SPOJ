...
{
	int numOfPackets,i;
	int candiesInPacketArr[10001];
	long long int sum,avgNumOfCandies,mod,candiesMoved,temp;
	...
	while(1)
	{
		...
		{
			scanf("%d",&candiesInPacketArr[i]);
			sum += candiesInPacketArr[i];
		}

		avgNumOfCandies = sum / numOfPackets;
		mod = sum % numOfPackets;
		...
		for( i = 0 ; i < numOfPackets ; i++ )
		{
			if( candiesInPacketArr[i] > avgNumOfCandies )
			{
				candiesMoved += ( candiesInPacketArr[i] - avgNumOfCandies );
				temp += ( candiesInPacketArr[i] - avgNumOfCandies );
			}
			else if( candiesInPacketArr[i] < avgNumOfCandies )
			{
				candiesMoved -= ( avgNumOfCandies - candiesInPacketArr[i] );
			}
		}
